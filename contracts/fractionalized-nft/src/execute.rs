    ...
    Q: CustomMsg,
{
    ...
    pub fn execute(
        ...
        match msg {
            ...
            ExecuteMsg::SetDepositDenom {
                denom,
            } => self.set_deposit_denom(deps, info, denom),
    ...
    Q: CustomMsg,
{
    ...
    pub fn buy_nfts(
        ...
        let deposit_denom = self.deposit_denom.load(deps.storage, "denom")?;

        if sent_token.denom != deposit_denom {
            return Err(ContractError::InvalidDenom {});
        }

        if sent_token.amount < (token.min_allocation * counts).into() {
            return Err(ContractError::Insufficient {});
        }

        if (counts + token.bought_counts) > (token.value / token.min_allocation) {
            return Err(ContractError::PurchaseOverflow {});
        }
    ...
    pub fn set_deposit_denom(
        ...
        cw_ownable::assert_owner(deps.storage, &info.sender)?;
        let _ = self.deposit_denom.save(deps.storage, "denom", &denom);
        Ok(Response::new()
